    ...
    return (
        <div className="w-full max-w-[90rem] animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                <button
                    onClick={() => navigate('/')}
                    className="text-slate-400 hover:text-white transition-colors flex items-center gap-2 self-start md:self-center"
                >
                ...
                <div className="text-center">
                    <h1 className="text-3xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-300">
                        Invest Smart
                    </h1>
                    <p className="text-slate-400 text-sm">Alavanque seu patrimônio com consórcio</p>
                </div>
                                            ...
                                            <div className="mt-2 pt-2 border-t border-slate-700/30">
                                                ...
                                                <div className="space-y-0.5 max-h-40 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
                                                    {result.periods.map((p, idx) => {
                                                        const isPre = p.start <= contemplationMonth && p.end < contemplationMonth;
                                                        const isPost = p.start > contemplationMonth;
                                                        // Accurate visual logic:
                                                        // Legacy: if start <= contemp && end < contemp -> Pre.
                                                        // What if the period CONTAINS the contemplation month (split)? 
                                                        // In logic, we split period at K. So p is either fully before or includes K or fully after.
                                                        // 'isPostContemplation' logic in loop: if m > K. 
                                                        // So any period starting > K is Post.
                                                        return (
